...
import SharedServicesSocket from "./Controllers/SocketsControllers/SharedServicesSocket.js";
...
export default (io, MongoClient) => {
    ...
    io.on('connection', async (socket) => {
        ...
        socket.on("cliente:tryAutch", async (data) => {
            ...
            if (validationMiddlewareResp == null) {
                ...
                socket.emit(`server:${data.accion.toString()}:init`, {
                    success: false,
                    code: "banned",
                    msj: session.back_list.reason
                })
                ...
                socket.emit(`server:${data.accion.toString()}:init`, {
                    success: false,
                    code: "unauthorized",
                    msj: "No estas autorizado para realizar esta accion."
                })
            }
    ...
    async function start(io, clientSocket, MongoClient, data) {
        ...
        SharedServicesSocket.run(io, clientSocket, MongoClient, {session, req: data});
